...
std::streamsize TriggerStrings::read(TriggerStrings::InputStream& istream)
{
	...
	for(Txt::Entries::const_reference ref : sectionEntries("TriggerEventStrings"))
	{
		readFunction(ref, this->events());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerConditionStrings"))
	{
		readFunction(ref, this->conditions());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerActionStrings"))
	{
		readFunction(ref, this->actions());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerCallStrings"))
	{
...
TriggerStrings::Entries& TriggerStrings::entries(TriggerFunction::Type type)
{
	...
	throw Exception(std::format(_("Unknown trigger function type: {}"), static_cast<int32>(type)));
}
