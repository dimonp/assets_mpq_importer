...
Blp::Format Blp::format(const byte *buffer, const std::size_t bufferSize)
{
	...
	{
		throw Exception(std::format(_("Error while reading BLP file. BLP identifier is too short: {}. Expected size of {}."), bufferSize, sizeof(dword)));
	}
	...
	*/
	...
	throw Exception(std::format(_("Error while reading BLP file. Missing BLP identifier, got \"{:08x}\"."), *reinterpret_cast<const uint32_t*>(buffer)));
}
...
{
	std::size_t mips = 0;
	std::size_t value = std::min<std::size_t>(width, height);
...
std::streamsize Blp::read(InputStream &istream,  const std::size_t &mipMaps)
{
	...
	std::size_t mipMapsCount = [&mipMapHeaders]() -> size_t {
		size_t prev_offset = 0;
		size_t count = 0;
		for(const auto& mipmap : mipMapHeaders) {
			if (mipmap.size == 0 || mipmap.offset == prev_offset) break;
			prev_offset = mipmap.offset;
			count++;
		}
		return count;
	}();
	...
	if (useCustomMipMapsCount)
	{
		...
		{
			std::cerr << std::format(_("Invalid specified number of MIP maps to be read: {0}. Maximum number is: {1}."), mipMaps, mipMapsCount) << std::endl;
		}
	}
	...
	switch (this->compression())
	{
		case Blp::Compression::Jpeg:
		{
			...
			{
				throw Exception(std::format(_("Too many bits in one single sample (one single pixel color channel): {0}. BLP/wc3lib allows maximum sample size of {1}."), BITS_IN_JSAMPLE, (sizeof(byte) * 8)));
			}
			...
			{
#if _DEBUG
				std::cerr << std::format(_("Warning: JPEG (JFIF) header size is not equal to 624 which is the usual size of Blizzard's JPEG compressed BLPs. It is {}."), jpegHeaderSize ) << std::endl;
#endif
			}
		...
		default:
		{
			std::cerr << std::format(_("Unsupported compression mode: {}."), static_cast<dword>(this->compression())) << std::endl;
...
std::streamsize Blp::write(OutputStream &ostream, int quality, std::size_t mipMaps, bool sharedHeader) const
{
	...
	if (quality < -1)
	{
		std::cerr << std::format(_("Invalid quality {0}. Minimum value is -1 which indicates using default quality."), quality) << std::endl;
		quality = -1;
	}
	else if (quality > 100)
	{
		std::cerr << std::format(_("Invalid quality {0}. Maximum value is 100 which indicates using best quality."), quality) << std::endl;
		quality = 100;
	}

	if (mipMaps > Blp::maxMipMaps)
	{
		std::cerr << std::format(_("Invalid MIP maps number {0}. Maximum value is {1}."), mipMaps, Blp::maxMipMaps) << std::endl;
	...
	*/
	...
	else if (this->format() == Blp::Format::Blp2)
	{
		...
		else
		{
			...
			{
				throw Exception(std::format(_("Unsupported compression for BLP2: {}"), static_cast<dword>(this->compression())));
			}
		}
	...
	switch (this->compression())
	{
		case Blp::Compression::Jpeg:
		{
			for (std::size_t i = 0; i < actualMipMaps; ++i)
			{
				...
				if (sharedHeader)
				{
					...
					{
						throw Exception(std::format(_("Invalid start of frame marker {}."), startOfFrameMarker));
					}
		...
		default:
		{
			std::cerr << std::format(_("Unknown compression mode: {}."), (dword)this->compression()) << std::endl;
