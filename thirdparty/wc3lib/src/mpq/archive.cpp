...
std::streamsize Archive::create(const std::filesystem::path &path, uint32 hashTableEntries, uint32 blockTableEntries, Format format, uint32 sectorSize, std::streampos startPosition)
{
	this->close();
	...
	if (!out)
	{
		throw Exception(std::format(_("Unable to create file \"{}\"."), path.string()));
	}
	...
	{
		throw Exception(std::format(_("Unable to start in file \"{}\" at position {}."), 
			path.string(), static_cast<size_t>(startPosition)));
	}
...
std::streamsize Archive::open(const std::filesystem::path &path)
{
	this->close();

	ifstream stream(std::filesystem::absolute(path), std::ios_base::binary | std::ios_base::in);

	if (!stream)
	{
		throw Exception(std::format(_("Unable to open file \"{}\"."), std::filesystem::absolute(path).string()));
	}

	this->m_path = std::filesystem::absolute(path);
	std::streamsize streamSize = 0;
...
std::streamsize Archive::read(InputStream &stream)
{
	...
	{
		throw Exception(std::format(_("Unknown MPQ format \"{}\"."), header.formatVersion));
	}

	if (header.blockTableEntries > maxBlockId + 1)
	{
		throw Exception(std::format(_("Too many block table entries ({}). Maximum allowed value is {} since {} and {} are reserved for deleted and empty block indices."), 
			header.blockTableEntries, maxBlockId, Hash::blockIndexDeleted, Hash::blockIndexEmpty));
	}
	...
	{
		std::cerr << std::format(_("Warning: Hash table entries should be a power of two for MPQ file {}.\nEntries: {}."), 
			this->path().string(), header.hashTableEntries) << std::endl;
	}
	...
	{
		std::cerr << std::format(_("Warning: There are too many MPQ hash table entries in MPQ file {}.\nEntries: {}.\nFor MPQ1 it must be less than 2^16 and for MPQ2 less than 2^20."), 
			this->path().string(), header.hashTableEntries) << std::endl;
	}
	...
	{
		std::cerr << std::format(_("Warning: MPQ header size is not equal to real header size.\nContained header size: {}.\nReal header size: {}."), header.headerSize, sizeof(header)) << std::endl;
	}

	// const uint64 expectedArchiveSize = std::filesystem::file_size(this->path()) - this->startPosition();
	auto end = endPosition(stream);
	const uint64 expectedArchiveSize = static_cast<uint64>(end) - this->startPosition();

	// if (header.archiveSize != expectedArchiveSize)
	if (header.archiveSize > expectedArchiveSize)
	{
		std::cerr << std::format(_(
			"Warning: MPQ file size of MPQ file {} is not equal to its internal header file size.\n"
			"File size: {}.\nInternal header file size: {}."),
			this->path().string(), expectedArchiveSize, header.archiveSize) << std::endl;
	}
...
bool Archive::readHeader(istream &in, Header &header, uint64 &startPosition, std::streamsize &size)
{
	...
	do
	{
		...
		if (currentSize < 4)
		{
			throw Exception(std::format(_("Missing identifier \"{}\" (read count {})."), Archive::identifier, size));
		}
...
bool Archive::readExtendedBlockTable(istream &in, std::streamsize &size)
{
	...
	for(Blocks::reference block : blocks)
	{
...
bool Archive::writeBlockTable(ostream &out, std::streamsize &size) const
{
	...
	for(Blocks::const_reference ref : this->blocks())
	{
...
bool Archive::writeHashTable(ostream &out, std::streamsize &size) const
{
	...
	for(Hashes::const_reference ref : this->hashes())
	{
...
File Archive::addFile(const std::filesystem::path &filePath, const byte *data, uint64 dataSize, Sector::Compression compression, Block::Flags flags, File::Locale locale, File::Platform platform)
{
	/*
	 ...
	 */
	for(Sector::Sectors::reference ref : sectors)
	{
...
Hash* Archive::findHash(const std::filesystem::path &path, File::Locale locale, File::Platform platform)
{
...
const Hash* Archive::findHash(const std::filesystem::path &path, File::Locale locale, File::Platform platform) const
{
	if (this->hashes().empty())
	{
		return 0;
...
File Archive::findFile(const std::filesystem::path &path, File::Locale locale, File::Platform platform)
{
