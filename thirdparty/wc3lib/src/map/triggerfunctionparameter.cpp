...
std::streamsize TriggerFunctionParameter::read(InputStream &istream, const TriggerData &triggerData)
{
	...
	{
		std::cerr << std::format(_("Warning: Functions in parameter which is not of type function itself - type {}."), static_cast<int32>(type())) << std::endl;
	}
	...
	{
		std::cerr << std::format(_("Warning: Array index count for non-variable parameter - type {}."), static_cast<int32>(type())) << std::endl;
	}
	else if (arrayIndexCount > 1 && type() == Type::Variable)
	{
		std::cerr << std::format(_("Warning: Array index count {} for variable parameter is greater than 1 although multi dimensional arrays are not supported."), arrayIndexCount) << std::endl;
	}
...
std::streamsize TriggerFunctionParameter::write(OutputStream &ostream) const
{
	std::streamsize size = 0;
	...
	for(Functions::const_reference ref : this->functions())
	{
