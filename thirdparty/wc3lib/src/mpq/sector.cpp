...
{
	ostream.seekp(static_cast<std::streampos>(archive()->startPosition()));
...
{
	istream.seekg(static_cast<std::streampos>(this->archive()->startPosition()));
...
namespace
{
	void sizeCheck(uint32 dataSize, const Sector &sector, const std::string &compression)
	{
		...
		{
			std::cerr << std::format(_("{}: Sector {} with size {} has same or smaller size after decompression: {}. Compression \"{}\"."), 
				"Sector", sector.sectorIndex(), sector.sectorSize(), dataSize, compression) << std::endl;
		}
	}
}
#endif
...
byte* Sector::compress(const byte *buffer, uint32 bufferSize, Block::Flags flags, Compression compression, uint32 &size, int waveCompressionLevel)
{
	...
	else if (flags & Block::Flags::IsCompressed)
	{
		...
		if (compression & Sector::Compression::ImaAdpcmMono) // IMA ADPCM mono
		{
			...
			if (outLength < boost::numeric_cast<int>(bufferSize))
			{
	...
	const bool uncompressed = data.get() == nullptr;
...
void Sector::decompress(byte *data, uint32 dataSize, ostream &ostream) const
{
	...
	if (compressionSucceded())
	{
		...
		else if (this->block()->flags() & Block::Flags::IsCompressed)
		{
			...
			if (this->compression() & Sector::Compression::Imploded)
			{
				//std::cerr << std::format(_("{}: Sector {} is imploded but file is not."), archive()->path(), sectorIndex()) << std::endl;
			...
			if (this->compression() & Sector::Compression::Huffman) // Huffman encoded
			{
				...
				if (state != 1)
				{
					throw Exception(std::format(_("Huffman error {}."), state));
				}
	...
	else
	{
		...
		{
#ifdef DEBUG
			std::cerr << std::format(_("{}: Sector {} with compressed size {} and uncompressed size {} could not be compressed properly. Archive sector size (uncompressed) is {}."),
				"Sector", sectorIndex(), this->sectorSize(), this->uncompressedSize(), archive()->sectorSize()) << std::endl;
#endif
		}
