...
void TriggerData::Function::fillTypes(TriggerData *triggerData, const SplitVector &values)
{
	for (std::size_t i = 0; i < values.size(); ++i)
	{
		...
		else
		{
			...
			{
				this->types().push_back(values[i]);
				std::cerr << std::format(_("Warning: Unknown type \"{}\" at index {} for trigger function \"{}\"."), values[i], i, this->code()) << std::endl;
			}
		}
	}
}
...
void TriggerData::Call::fillTypes(TriggerData *triggerData, const SplitVector &values)
{
	...
	{
		std::cerr << std::format(_("Missing \"canBeUsedInEvents\" value for trigger call \"{}\"."), this->code()) << std::endl;
	}
}
...
void TriggerData::readFunction(const Txt::Entry &ref, boost::ptr_map<string, FunctionType> &functions)
{
	...
	else
	{
		...
		{
			std::cerr << std::format(_("Missing suffix for trigger function entry \"{}\"."), ref.first) << std::endl;
		}
		...
		{
			std::cerr << std::format(_("Missing base trigger function \"{}\" for trigger function entry \"{}\"."), code, ref.first) << std::endl;
	...
	else if (ref.first.substr(index + 1) == "Defaults")
	{
		if (!function->defaults().empty())
		{
			...
			std::cerr << std::format(_("Redefinition of defaults for trigger function \"{}\"."), code) << std::endl;
		}
	...
	else if (ref.first.substr(index + 1) == "Category")
	{
		if (values.size() >= 1)
		{
			...
			{
				std::cerr << std::format(_("Invalid category \"{}\" for trigger function \"{}\"."), values[0], code) << std::endl;
			}
		}
		else
		{
			std::cerr << std::format(_("Empty category for trigger function \"{}\"."), code) << std::endl;
		}
	}
	else if (ref.first.substr(index + 1) == "Limits")
	{
		if (!function->limits().empty())
		{
			...
			std::cerr << std::format(_("Redefinition of limits for trigger function \"{}\"."), code) << std::endl;
		}
		...
		for (std::size_t i = 0; i < values.size(); ++i)
		{
			...
			*/
			...
			{
				std::cerr << std::format(_("Limits of function \"{}\" miss maximum value for limit {}"), code, function->limits().size()) << std::endl;
			}
...
std::streamsize TriggerData::read(InputStream &istream)
{
	...
	for(Txt::Entries::const_reference ref : sectionEntries("TriggerCategories"))
	{
	...
	for(Txt::Entries::const_reference ref : sectionEntries("TriggerTypes"))
	{
		...
		if (values.size() >= 4)
		{
			...
			std::cerr << std::format(_("Missing base type for type \"{}\"."), name) << std::endl;
		}*/
		...
		if (values.size() > 4)
		{
			throw Exception(std::format(_("Detected The Frozen Throne type in a Reign of Chaos TriggerData.txt file for type \"{}\"."), name));
		}
	...
	for(BaseTypes::reference ref : baseTypes)
	{
		...
		if (baseTypeIterator != this->types().end())
		{
			...
			{
				std::cerr << std::format(_("Missing type \"{}\" for base type \"{}\"."), type, baseType) << std::endl;
			}
		}
		else
		{
			std::cerr << std::format(_("Missing base type \"{}\" for type \"{}\"."), baseType, type) << std::endl;
		}
	}

	// set trigger type defaults which are stored in a separated category
	for(Txt::Entries::const_reference ref : sectionEntries("TriggerTypeDefaults"))
	{
		...
		{
			std::cerr << std::format(_("Missing default type \"{}\"."), ref.first) << std::endl;
		}
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerParams"))
	{
	...
	for(Txt::Entries::const_reference ref : sectionEntries("TriggerEvents"))
	{
		readFunction<Function>(ref, this->events());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerConditions"))
	{
		readFunction<Function>(ref, this->conditions());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerActions"))
	{
		readFunction<Function>(ref, this->actions());
	}

	for(Txt::Entries::const_reference ref : sectionEntries("TriggerCalls"))
	{
	...
	for(Txt::Entries::const_reference ref : sectionEntries("DefaultTriggerCategories"))
	{
		...
		else if (boost::starts_with(ref.first, "Category"))
		{
			...
			{
				std::cerr << std::format(_("Invalid index {} for default trigger category \"{}\"."), index, ref.second) << std::endl;
			}
	...
	{
		std::cerr << std::format(_("Expected default trigger categories {} are not equal to actual ones {}."), numCategories, actualCategories) << std::endl;
	}

	std::size_t numTriggers = 0;
	std::size_t actualTriggers = 0;

	for(Txt::Entries::const_reference ref : sectionEntries("DefaultTriggers"))
	{
		...
		else if (boost::starts_with(ref.first, "Trigger"))
		{
			...
			{
				std::cerr << std::format(_("Default trigger entry \"{}\" is missing type."), ref.first) << std::endl;
			...
			if (index < this->defaultTriggers().size())
			{
				...
				else if (type == "Category")
				{
					...
					{
						throw Exception(std::format(_("Default trigger category {} does not exist but is used by default trigger {}."), (category + 1), index));
					}
				}
				else if (type == "Events")
				{
				...
				else if (boost::starts_with(type, "Event"))
				{
					...
					if (iterator != this->events().end())
					{
						...
						{
							std::cerr << std::format(_("Invalid event index {} for default trigger entry {}."), eventIndex, index) << std::endl;
						}
					}
					else
					{
						std::cerr << std::format(_("Default trigger entry {} has invalid event value \"{}\"."), index, ref.second) << std::endl;
					}
				}
				else if (boost::starts_with(type, "Condition"))
				{
					...
					if (iterator != this->conditions().end())
					{
						...
						{
							std::cerr << std::format(_("Invalid condition index {} for default trigger entry {}."), conditionIndex, index) << std::endl;
						}
					}
					else
					{
						std::cerr << std::format(_("Default trigger entry {} has invalid condition value \"{}\"."), index, ref.second) << std::endl;
					}
				}
				else if (boost::starts_with(type, "Action"))
				{
					...
					if (iterator != this->actions().end())
					{
						...
						{
							std::cerr << std::format(_("Invalid action index {} for default trigger entry {}."), actionIndex, index) << std::endl;
						}
					}
					else
					{
						std::cerr << std::format(_("Default trigger entry {} has invalid action value \"{}\"."), index, ref.second) << std::endl;
					}
				}
				else
				{
					std::cerr << std::format(_("Default trigger entry \"{}\" has unknown suffix."), type) << std::endl;
				}
			}
			else
			{
				std::cerr << std::format(_("Invalid index {} for default trigger \"{}\"."), index, ref.first) << std::endl;
			}
		}
	}
	...
	{
		std::cerr << std::format(_("Expected default triggers {} are not equal to actual ones {}."), numTriggers, actualTriggers) << std::endl;
	}
...
std::streamsize TriggerData::write(OutputStream &ostream) const
{
	...
	for(Categories::const_reference ref : this->categories())
	{
	...
	for(Types::const_reference ref : this->types())
	{
	...
	for(Parameters::const_reference ref : this->parameters())
	{
	...
	for(Functions::const_reference ref : this->events())
	{
	...
	for(Functions::const_reference ref : this->conditions())
	{
	...
	for(Functions::const_reference ref : this->actions())
	{
	...
	for(Calls::const_reference ref : this->calls())
	{
	...
	for(DefaultTriggerCategories::const_reference ref : this->defaultTriggerCategories())
	{
		Txt::Entry categoryEntry;
		stringstream sstream;
		sstream << "Category";
	...
	for(DefaultTriggers::const_reference ref : this->defaultTriggers())
	{
		stringstream sstream;
		sstream << "Trigger";
		...
		for(DefaultTrigger::Functions::const_reference eventRef : ref.events())
		{
			sstream.str("");
		...
		for(DefaultTrigger::Functions::const_reference conditionRef : ref.conditions())
		{
			sstream.str("");
		...
		for(DefaultTrigger::Functions::const_reference actionRef : ref.actions())
		{
			sstream.str("");
...
const Txt::Entries& TriggerData::sectionEntries(const string &sectionName) const
{
	...
	throw Exception(std::format(_("Unknown section: {}"), sectionName));
}
