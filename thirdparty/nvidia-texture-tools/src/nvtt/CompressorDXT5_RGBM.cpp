...
float nv::compress_dxt5_rgbm(const Vector4 input_colors[16], const float input_weights[16], float min_m, BlockDXT5 * output) {
    ...
    // Convert to RGBM.
    alignas(32) Vector4 input_colors_rgbm[16]; // @@ Write over input_colors?
    ...
    // Compress RGB.
    icbc::compress_dxt1(icbc::Quality_Default, input_colors_rgbm, rgb_weights, color_weights, /*three_color_mode=*/false, /*hq=*/false, &output->color);
