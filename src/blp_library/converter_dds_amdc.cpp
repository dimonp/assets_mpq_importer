...
// cppcoreguidelines-avoid-non-const-global-variables, cppcoreguidelines-pro-type-reinterpret-cast)
...
auto persist_dds_dx10(const MipSet &mipSet) -> std::vector<char>
{
    ...
    DDS_FILE_HEADER_DXT10 HeaderDDS10 = { .dxgiFormat = get_dxgi_format(mipSet),
...
static auto generate_extra_mipmaps(MipSet &mipset_in,
  ...
  const size_t last_mipmap_idx) -> bool
{
    ...
    while (mip_width > 1 || mip_height > 1) {
        ...
        GenerateMipmapLevel(this_mip_level, &prev_mip_level, 1, mipset_in.m_format);
    }
    return true;
}


// NOLINTEND(clang-diagnostic-missing-designated-field-initializers, cppcoreguidelines-avoid-non-const-global-variables,
...
try {
    ...
    for (size_t mip_idx = 0; mip_idx < mipmap_count; ++mip_idx) {
        ...
        if (texture.compression() == wc3lib::blp::Blp::Compression::Paletted) {
            ...
            mipmap_color_buffer = utils::get_paletted_mipmap_buffer_rgba(mipmap, palette);
        } else {
            mipmap_color_buffer = utils::get_mipmap_buffer_rgba(mipmap);
        }
    ...
    if (extra_mipmaps > 0) {
        const bool result = generate_extra_mipmaps(*mipset_in, texture.mipMaps()[mipmap_count - 1], mipmap_count - 1);

        if (!result) { return std::unexpected("Compressionator: Error generating extra mipmaps."); }
    }
    ...
    if (CMP_ProcessTexture(
        mipset_in.get(),
        mipset_out.get(),
        options,
        dont_stop_callback) != CMP_OK) {
