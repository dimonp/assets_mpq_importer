...
std::streamsize Info::RandomUnitTable::Group::write(OutputStream &ostream) const
{
	...
	for(Columns::const_reference column : columns())
	{
	...
	for(Chances::const_reference chance : chances())
	{
...
std::streamsize Info::RandomUnitTable::write(OutputStream &ostream) const
{
	...
	for(Groups::const_reference group : groups())
	{
...
std::streamsize Info::read(InputStream &istream)
{
	...
	{
		std::cerr << std::format(_("Info: Unsupported version {}. Latest version is {}."), version(), latestFileVersion()) << std::endl;
	}
...
std::streamsize Info::write(OutputStream &ostream) const
{
	...
	for(Players::const_reference player : players())
	{
	...
	for(Forces::const_reference force : forces())
	{
	...
	for(UpgradeAvailabilities::const_reference upgradeAvailability : upgradeAvailabilities())
	{
	...
	for(TechAvailabilities::const_reference techAvailability : techAvailabilities())
	{
	...
	for(RandomUnitTables::const_reference randomUnitTable : randomUnitTables())
	{
