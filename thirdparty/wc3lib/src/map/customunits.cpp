...
std::streamsize CustomUnits::Modification::read(InputStream &istream)
{
	...
	//{
		...
		if (end != 0)
		{
			std::cerr << std::format(_("Modification \"{}\" with type {} end byte is not 0: {}"), this->valueId(), static_cast<int32>(this->value().type()), end) << std::endl;
		}
	//}
...
std::streamsize CustomUnits::Modification::readData(InputStream &istream, int32 t, Value::Type type)
{
	...
	switch (type)
	{
		...
		default:
		{
			std::cerr << std::format(_("Unknown value type {} for modification {}."), t, idToString(this->m_id)) << std::endl;
...
{
	for(Modifications::const_reference ref : other.m_modifications)
	{
...
std::streamsize CustomUnits::Set::write(OutputStream &ostream) const
{
	...
	for(Modifications::const_reference modification : this->modifications())
	{
...
{
	for(Sets::const_reference ref : other.m_sets)
	{
...
std::streamsize CustomUnits::Unit::write(OutputStream &ostream) const
{
	...
	for(Sets::const_reference set : this->sets())
	{
...
CustomUnits::CustomUnits(const CustomUnits &other)
{
	...
	for(Table::const_reference ref : other.originalTable())
	{
	...
	for(Table::const_reference ref : other.customTable())
	{
		// call polymorphic clone method which might also create a new custom object for custom objects
...
std::streamsize CustomUnits::write(OutputStream &ostream) const
{
	...
	for(Table::const_reference unit : originalTable())
	{
	...
	for(Table::const_reference unit : customTable())
	{
		size += unit.write(ostream);
	}
