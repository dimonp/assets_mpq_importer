...
inline void checkStream(std::basic_istream<_CharT> &stream)
{
	if (!stream)
	{
		...
		throw Exception(std::format(_("Stream error.\nExceptions \"{}\".\nRD state: \"{}\".\nRead pos: \"{}\".\nRead count: \"{}\"."), 
			iostateMessage(stream.exceptions()), iostateMessage(stream.rdstate()), s.str(), stream.gcount()));
	}
}
...
inline void checkStream(std::basic_ostream<_CharT> &stream)
{
	if (!stream)
	{
		...
		throw Exception(std::format(_("Stream error.\nExceptions \"{}\".\nRD state: \"{}\".\nWrite pos: \"{}\"."),
			iostateMessage(stream.exceptions()), iostateMessage(stream.rdstate()), s.str()));
	}
}
...
inline std::basic_istream<_CharT>& read(std::basic_istream<_CharT> &istream, T &value, std::streamsize &sizeCounter, std::streamsize size = sizeof(T) * sizeof(_CharT))
{
	...
	{
		throw Exception(std::format(_("Input stream read size {} is not equal to expected size {}."), istream.gcount(), size));
	}
...
inline std::basic_istream<_CharT>& readVector(std::basic_istream<_CharT> &istream, std::vector<T> &value, std::streamsize &sizeCounter)
{
	...
	if (size <= 0)
	{
		throw Exception(std::format(_("Expected size {} is not bigger than 0."), size));
	}
	...
	do
	{
		...
		if (character != terminatingChar)
		{
			...
			*/
			if (length == maxLength)
			{
				throw Exception(std::format(_("0 terminated string from stream is longer that maximum length {}."), maxLength));
			}
		}
		else
		{
...
std::string sizeStringBinary(T size)
{
	...
	{
		static_cast<SizeType>(pow(1024, 3)), // TODO compile time power
		static_cast<SizeType>(pow(1024, 2)), // TODO compile time power
		1024,
		0
	};
...
std::string sizeStringDecimal(T size)
{
	...
	static const SizeType table[tableSize] =
	{
		static_cast<SizeType>(pow(1000, 3)), // TODO compile time power
		static_cast<SizeType>(pow(1000, 2)), // TODO compile time power
		1000,
		0
	};
	static const char *identifierTable[tableSize] =
