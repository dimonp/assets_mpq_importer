...
std::streamsize TriggerFunction::read(InputStream &istream, const TriggerData &triggerData)
{
	...
	switch (this->type())
	{
		case TriggerFunction::Type::Event:
		{
			...
			{
				throw Exception(std::format(_("Function \"{}\" not found in events of TriggerData.txt."), this->name()));
			}
		...
		case TriggerFunction::Type::Condition:
		{
			...
			{
				throw Exception(std::format(_("Function \"{}\" not found in conditions of TriggerData.txt."), this->name()));
			}
		...
		case TriggerFunction::Type::Action:
		{
			...
			{
				throw Exception(std::format(_("Function \"{}\" not found in actions of TriggerData.txt."), this->name()));
			}
		...
		default:
		{
			throw Exception(std::format(_("Function type {} is invalid."), static_cast<int32>(this->type())));
		}
	}
...
std::streamsize TriggerFunction::write(OutputStream &ostream) const
{
	...
	for(Parameters::const_reference value : this->parameters())
	{
