...
std::streamsize ImportedFiles::Path::read(InputStream &istream)
{
	...
	else
	{
		...
		std::cerr << std::format(_("Imported files: Invalid path byte {}."), hasPrefix) << std::endl;
	}
...
std::streamsize ImportedFiles::read(InputStream &istream)
{
	...
	{
		std::cerr << std::format("Imported files: Unsupported version {}, expected {}", this->version(), latestFileVersion()) << std::endl;
	}
...
std::streamsize ImportedFiles::write(OutputStream &ostream) const
{
	...
	for(Paths::const_reference path : this->paths())
	{
