...
static auto wildcard_to_regex(std::string mask) -> re2::RE2
{
    ...
    std::size_t pos = 0;
    while ((pos = mask.find('.', pos)) != std::string::npos) {
        mask.replace(pos, 1, "\\.");
        pos += 2;
    }
    pos = 0;
...
auto list_mpq_files(const std::filesystem::path &archive_path, std::string_view mask)
  -> std::expected<ArchiveEntries, ErrorMessage>
{
    ...
    if (archive.containsListfileFile()) {
        ...
        if (filelist.isValid()) {
            const auto mask_regex = wildcard_to_regex(static_cast<std::string>(mask));
            ...
            return entries | std::views::filter(filter)
...
auto extract_mpq_file(const std::filesystem::path &archive_path, std::string_view filename)
...
try {
    ...
    if (!file.isValid()) { return std::unexpected("File not found."); }
