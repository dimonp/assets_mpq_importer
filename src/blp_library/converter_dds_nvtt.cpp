    ...
    const size_t last_mipmap_idx) -> bool
{
    ...
    if (texture.compression() == wc3lib::blp::Blp::Compression::Paletted) {
        ...
        mipmap_color_buffer = utils::get_paletted_mipmap_buffer_rgba(last_mipmap, palette);
    } else {
        mipmap_color_buffer = utils::get_mipmap_buffer_rgba(last_mipmap);
    }
    ...
    while (surface.buildNextMipmap(nvtt::MipmapFilter_Triangle, 1)) {
        context.compress(surface, 0, static_cast<int>(++mip_idx), compression_options, output_options);
    }

    return true;
}

auto convert_blp_to_dds_texture_nvtt(const FileData &blp_file, const Compression &compression, bool regen_mipmaps)
...
try {
    ...
    const auto max_mipmaps = nv::countMipmaps(static_cast<unsigned>(blp_width), static_cast<unsigned>(blp_height), 0);
    ...
    if (context.outputHeader(nvtt::TextureType_2D,
        ...
        for (size_t mip_idx = 0; mip_idx < mipmap_count; ++mip_idx) {
            ...
            if (texture.compression() == wc3lib::blp::Blp::Compression::Paletted) {
                const auto &palette = texture.palette();
                mipmap_color_buffer = utils::get_paletted_mipmap_buffer_float(mipmap, palette);
            } else {
                mipmap_color_buffer = utils::get_mipmap_buffer_float(mipmap);
            }
