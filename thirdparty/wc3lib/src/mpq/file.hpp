...
class File
{
	public:
		 ...
		 */
		virtual std::streamsize decompress(ostream &ostream) const;
		/**
		 ...
		 */
		virtual std::streamsize decompress(istream &istream, ostream &ostream) const;
		 ...
		 */
		std::streamsize sectors(istream &istream, Sector::Sectors &sectors) const;
		/**
		 * Reads the file's sector table into \p sectors.
		 *
		 * \return Returns the number of bytes read.
		 */
		std::streamsize sectors(Sector::Sectors &sectors) const;
	...
	protected:
		 ...
		 */
		File(Archive *mpq, Hash *hash, const std::filesystem::path &path);
		 ...
		 */
		void changePath(const std::filesystem::path &path);
...
inline const std::filesystem::path& File::path() const
{
	return this->m_path;
}
...
inline std::filesystem::path File::nativePath() const
{
#ifdef UNIX
