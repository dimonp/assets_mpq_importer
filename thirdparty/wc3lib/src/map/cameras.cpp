...
std::streamsize Cameras::read(InputStream &istream)
{
	...
	{
		std::cerr << std::format(_("Cameras: Unknown version \"{}\", expected \"{}\"."), this->version(), latestFileVersion()) << std::endl;
	}
...
std::streamsize Cameras::write(std::ostream &ostream) const
{
	...
	{
		std::cerr << std::format(_("Cameras: Unknown version \"%1%\", expected \"%2%\"."), this->version(), latestFileVersion()) << std::endl;
	}
	...
	for(CameraContainer::const_reference camera : this->cameras())
	{
