...
auto load_model(const std::string& path, assmpq::merger::MeshGroups& mesh_groups)-> bool
{
    ...
    const aiScene *scene_ptr = importer.ReadFile(path,
        ...
        static_cast<unsigned>(aiProcess_RemoveRedundantMaterials));
    ...
    for (size_t i = 0; i < scene_ptr->mNumMeshes; ++i) {
        ...
        meshData.aabb = {
            .min = {
                mesh_ptr->mAABB.mMin.x,
                mesh_ptr->mAABB.mMin.y,
                mesh_ptr->mAABB.mMin.z
            },
            .max = {
                mesh_ptr->mAABB.mMax.x,
                mesh_ptr->mAABB.mMax.y,
                mesh_ptr->mAABB.mMax.z
            }
        };
...
auto save_model(const std::string& path, const MeshGroups& mesh_groups, bool generate_normals)-> bool
{
    ...
    for(size_t i = 0; i < mesh_groups.size(); ++i) {
        ...
        for(size_t j = 0; j < meshData.vertices.size(); ++j) {
            mesh->mVertices[j] = {
                meshData.vertices[j].x,
                meshData.vertices[j].y,
                meshData.vertices[j].z,
            };
        }
        ...
        if(!meshData.normals.empty()) {
            ...
            for(size_t j = 0; j < meshData.vertices.size(); ++j) {
                mesh->mNormals[j] = {
                    meshData.normals[j].x,
                    meshData.normals[j].y,
                    meshData.normals[j].z,
                };
            }
        } else {
