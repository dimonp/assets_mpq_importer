...
namespace nv
{
    ...
    */
    ...
    class AtomicAutoPtr
    {
        NV_FORBID_COPY(AtomicAutoPtr);
        NV_FORBID_HEAPALLOC();
    public:

        /// Ctor.
        AtomicAutoPtr(T * p = NULL) : m_ptr(p) { }

        template <class Q>
        AtomicAutoPtr(Q * p) : m_ptr(static_cast<T *>(p)) { }

        AtomicAutoPtr(AtomicAutoPtr&& other) {
            m_ptr = other.m_ptr;
            other.m_ptr = nullptr;
        }

        AtomicAutoPtr& operator=(AtomicAutoPtr&& other) {
            if (m_ptr == other.m_ptr) {
                return *this;
            }
            delete m_ptr;
        ...
        /// Const pointer equal comparation.
        friend bool operator == (const AtomicAutoPtr<T> & ap, const T * const p) {
            return (ap.ptr() == p);
        }
        ...
        /// Const pointer equal comparation.
        friend bool operator == (const T * const p, const AtomicAutoPtr<T> & ap) {
            return (ap.ptr() == p);
        }
