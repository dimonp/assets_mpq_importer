...
class Archive : public Format, private boost::noncopyable
{
	public:
		 ...
		 */
		std::streamsize create(const std::filesystem::path &path, uint32 hashTableEntries, uint32 blockTableEntries, Format format = Format::Mpq1, uint32 sectorSize = 4096, std::streampos startPosition = 0);
		/**
		 * Opens an MPQ file on the local file system at \p path.
		 * This reads the block and hash tables.
		 */
		std::streamsize open(const std::filesystem::path &path);
		/**
		...
		*/
		 ...
		 */
		File addFile(const std::filesystem::path &filePath, const byte *data, uint64 dataSize, Sector::Compression compression = Sector::Compression::Uncompressed, Block::Flags flags = Block::Flags::None, File::Locale locale = File::Locale::Neutral, File::Platform platform = File::Platform::Default);
		 ...
		 * @{
		 */
		Hash* findHash(const std::filesystem::path &path, File::Locale locale = File::Locale::Neutral, File::Platform platform = File::Platform::Default);
		const Hash* findHash(const std::filesystem::path &path, File::Locale locale = File::Locale::Neutral, File::Platform platform = File::Platform::Default) const;
		/**
		 * @}
		 */
		 ...
		 * \ingroup search
		 */
		File findFile(const std::filesystem::path &path, File::Locale locale = File::Locale::Neutral, File::Platform platform = File::Platform::Default);
...
{
	static const std::array<uint8, 4> signaturue_identifier = { 'N', 'G', 'I', 'S' };
	uint32 data = 0;
	...
	return size == sizeof(signaturue_identifier) &&
		memcmp(signaturue_identifier.data(), &data, sizeof(signaturue_identifier)) == 0;
}
...
inline const std::filesystem::path& Archive::path() const
{
	return this->m_path;
}
...
{
	return static_cast<uint16>(sqrt(this->m_sectorSize / 512));
}
...
{
	for(Hashes::value_type pair : this->hashes())
	{
