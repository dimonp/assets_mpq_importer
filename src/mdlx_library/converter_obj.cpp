...
static void process_scene(const wc3lib::mdlx::Mdlx &model, aiScene &scene, std::string_view mesh_name)
{
    ...
    for (size_t i = 0; i < scene.mNumMaterials; ++i) { scene.mMaterials[i] = new aiMaterial; }
    ...
    for (size_t geoset_idx = 0; geoset_idx < geoset_count; ++geoset_idx) {
        ...
        if (geoset_count > 1) {
            ...
            mesh_ptr->mName = static_cast<std::string>(mesh_name);
        }
