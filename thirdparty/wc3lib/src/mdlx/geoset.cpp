...
std::streamsize Geoset::read(InputStream &istream)
{
	...
	wc3lib::read(istream, m_materialId, size);
	wc3lib::read(istream, m_selectionGroup, size);
	wc3lib::read(istream, m_selectable, size);
	...
	// ganimations[nganim]
	long32 nanim = 0;
	wc3lib::read(istream, nanim, size);
	m_ganimations.reserve(nanim);
	for (long32 i = 0; i < nanim; i++) {
		Bounds bounds;
		size += bounds.read(istream);
		m_ganimations.emplace_back(bounds);
	}

	wc3lib::read(istream, tag, size);
	expectMdxTag(istream, tag, u8"UVAS");
	long32 ntvrts = 0;
	wc3lib::read(istream, ntvrts, size);

	m_texturePatches.reserve(ntvrts);
	for (long32 i = 0; i < ntvrts; i++) {
		wc3lib::read(istream, tag, size);
		expectMdxTag(istream, tag, u8"UVBS");
		long32 nvrts = 0;
		wc3lib::read(istream, nvrts, size);


		TextureVertices textureVertices;
		textureVertices.reserve(nvrts);
		for (long32 j = 0; j < nvrts; j++) {
			Vertex2d<float32> uv;
			size += uv.read(istream);
			textureVertices.emplace_back(uv);
		}

		m_texturePatches.emplace_back(textureVertices);
	}

    // TODO Support all blocks.

    skipMdxInclusiveEmptyBytes(istream, nbytes, size);
