		...
		if (buffer[i] == 0xFF)
		{
			...
			if (j < bufferSize && buffer[j] == marker)
			{
				if (variableMarkerSize || markerSize > 0)
				{
					...
					{
						throw Exception(std::format(_("JPEG marker \"%1%\" needs more data to get its own data size."), hexValue(marker)));
					}
				...
				{
					throw Exception(std::format(_("JPEG marker \"{}\" needs more data with marker size {} and data size {}, starting at {}."), hexValue(marker), markerSize, bufferSize, (i + 2)));
				}
	...
	for (std::size_t i = 0; i < bufferSize && !found; ++i)
	{
		if (buffer[i] == 0xFF)
		{
			...
			if (j < bufferSize && buffer[j] == marker)
			{
				...
				if (j + 1 > bufferSize) // we need 2 bytes for marker size (which could be 0 as well)
				{
					throw Exception(std::format(_("JPEG marker \"{}\" needs more data to get its own data size."), hexValue(marker)));
				}
				//memcpy(&markerSize, &(buffer[j]), sizeof(markerSize));
...
std::string jpegError(std::string_view message)
{
	const char *error = jpeg_std_error(0)->jpeg_message_table != 0
                        ? jpeg_std_error(0)->jpeg_message_table[jpeg_std_error(0)->last_jpeg_message]
                        : _("No error");

	return std::vformat(message, std::make_format_args(error));
}
...
void readMipMapJpeg(Blp::MipMap &mipMap, byte *buffer, dword bufferSize)
{
	...
	try
	{
		...
		{
			throw Exception(jpegError(_("Did not find header. Error: {}.")));
		}

		if (!jpeg_start_decompress(&cinfo))
		{
			throw Exception(jpegError(_("Could not start decompress. Error: {}.")));
		}

		if (mipMap.width() != cinfo.image_width)
		{
			std::cerr << std::format(_("Corrupted MIP map: Image width ({}) is not equal to mip map width ({})."), cinfo.image_width, mipMap.width()) << std::endl;
		...
		{
			std::cerr << std::format(_("Corrupted MIP map: Image height ({}) is not equal to mip map height ({})."), cinfo.image_height, mipMap.height()) << std::endl;
...
#if _DEBUG
				std::cerr << std::format(_("Number of scanned lines is not equal to {}. It is {}."), requiredScanlines, dimension) << std::endl;
#endif
			}
...
void writeMipMapJpeg(const Blp::MipMap &mipMap, unsigned char *&buffer, unsigned long &bufferSize, int quality)
{
	...
	try
	{
		...
		while (cinfo.next_scanline < cinfo.image_height)
		{
			...
			if (dimension != requiredScanlines)
			{
				throw Exception(std::format(_("Number of written scan lines is not equal to {}. It is {}."), requiredScanlines, dimension));
			}
		}
	}
	/*
