...
std::streamsize Environment::read(InputStream &istream)
{
	...
	if (groundTilesetsNumber > maxTilesets)
		std::cerr << std::format(_("Environment (warning): Only the first {} ground tilesets can be used, however {} are stored."), maxTilesets, groundTilesetsNumber) << std::endl;
	...
	if (cliffTilesetsNumber > maxTilesets)
		std::cerr << std::format(_("Environment (warning): Only the first {} cliff tilesets can be used, however {} are stored."), maxTilesets, groundTilesetsNumber) << std::endl;
...
std::streamsize Environment::write(OutputStream &ostream) const
{
	...
	if (groundTilesetsIds().size() > maxTilesets)
		std::cerr << std::format(_("Environment (warning): Only the first {} ground tilesets can be used, however {} are stored."), maxTilesets, groundTilesetsIds().size()) << std::endl;
	...
	if (cliffTilesetsIds().size() > maxTilesets)
		std::cerr << std::format(_("Environment (warning): Only the first {} ground tilesets can be used, however {} are stored."), maxTilesets, cliffTilesetsIds().size()) << std::endl;
