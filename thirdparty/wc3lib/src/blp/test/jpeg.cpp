...
TEST_CASE("WriteHeader", "[jpeg]")
{
	...
	try
	{
		const auto imageWidth = boost::numeric_cast<JDIMENSION>(64);
		const auto imageHeight = boost::numeric_cast<JDIMENSION>(64);
	...
	CHECK(hasJpegMarker(0xD8, buffer, bufferSize));
	// no JFIF marker and no comment!
	CHECK(hasJpegMarker(0xDB, buffer, bufferSize));
	CHECK(hasJpegMarker(0xC4, buffer, bufferSize));
	CHECK(bufferSize > 0);
}

TEST_CASE("WriteHeader2", "[jpeg]")
{
	...
	std::ofstream jpegheaderout("jpegheaderout", std::ios::binary | std::ios::out);
	jpegheaderout.write(reinterpret_cast<char*>(buffer), bufferSize);
	jpegheaderout.close();

	std::ofstream ostream("tmp", std::ios::binary | std::ios::out);
	 ...
	 */
	std::ifstream istream("tmp", std::ios::binary | std::ios::in);
	...
	// istream.read(reinterpret_cast<char*>(buffer2), headerSize);
	// istream.close();
	...
	// 	if (i >= sizeof(buffer2))
	// 	{
	// 		std::cerr << std::hex << "Byte in jpeglib header but not in custom: " << static_cast<int>(buffer[i]) << std::endl;
	// 	}

	// 	if (i >= bufferSize)
	// 	{
	// 		std::cerr << std::hex << "Byte in custom header but not in jpeg: " << static_cast<unsigned int>(buffer2[i]) << std::endl;
	// 	}
	// }
