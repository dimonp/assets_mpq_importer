...
std::streamsize Sound::read(InputStream &istream)
{
	...
	if (eax.empty()) {
		...
		throw Exception(std::format(_("Sound: Unknown EAX effect \"{}\"."), eax));
...
std::streamsize Sound::write(OutputStream &ostream) const
{
	std::streamsize size = 0;
