...
std::streamsize W3m::readFileFormat(FileFormat *format)
{
	...
	if (file.isValid())
	{
		...
		*/
		...
		{
			throw Exception(std::format(_("Error while reading map file \"{}\":\n{}"), format->fileName(), exception.what()));
		}
	}
...
std::streamsize W3m::readTriggers(const TriggerData &triggerData)
{
	...
	if (file.isValid())
	{
		...
		*/
		...
		{
			throw Exception(std::format(_("Error while reading map file \"{}\":\n{}"), m_triggers.get()->fileName(), exception.what()));
		}
	}
	else
	{
		throw Exception(std::format(_("Couldn't find \"{}\""), m_triggers.get()->fileName()));
	}
...
std::streamsize W3m::readHeader(InputStream &istream)
{
	...
	{
		throw Exception(std::format(_("W3m: Unknown file id \"{}\". Expected \"{}\"."), fileId, this->fileId()));
	}
